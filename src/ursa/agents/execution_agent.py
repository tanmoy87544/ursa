...
@tool
def write_code(code: str, filename: str, tool_call_id: Annotated[str, InjectedToolCallId], state: Annotated[dict, InjectedState]) -> Command:
    ...
    try:
        ...
        console.print("[bold bright_white on red] :heavy_multiplication_x: [/] "
                      "[red]Failed to write file:[/]", exc)
        return f"Failed to write {filename}."

    console.print(f"[bold bright_white on green] :heavy_check_mark: [/] "
...
def main():
    ...
    result = execution_agent.action.invoke(inputs, {"configurable": {"thread_id": execution_agent.thread_id}})
